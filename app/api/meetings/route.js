...
export async function PATCH(req) {
  try {
    ...
    console.log("Meeting berhasil diubah dengan ID:", meeting.id);
      ...
      await Promise.all(
        batch.map(async (attendee) => {
          ...
          if (user.settings?.meetingReminder) {
            await createNotification({
              userId: user.id,
              title: "Perubahan Meeting",
              message: `Meeting "${meeting.title} mengalami beberapa perubahan, silahkan cek lebih lanjut!"`,
              meetingId: meeting.id,
            });
          }
          ...
          if (kirimUndanganEmail && user.settings?.emailNotification) {
            await sendEmail({
              to: user.email,
              subject: `Perubahan Meeting: ${meeting.title}`,
              html: `
    <!DOCTYPE html>
    <html lang="id">
    <head>
      <meta charset="UTF-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>Perubahan Meeting - KAI Rooms</title>
      <style>
        body {
          font-family: 'Inter', 'Segoe UI', -apple-system, BlinkMacSystemFont, sans-serif;
          line-height: 1.6;
    ...
    <body>
      <div class="container">
        <div class="header">
          <div class="logo">🚆 KAI Rooms</div>
          <h1>Perubahan Jadwal Meeting</h1>
        </div>
        ...
        <div class="content">
          ...
          <p style="color: #4b5563; font-size: 16px; line-height: 1.6;">
            Meeting Anda yang berjudul ${
              meeting.title
            } silahkan cek lebih lanjut melalui sistem KAI Rooms.
          </p>
    ...
    return new Response(
      JSON.stringify({ message: "Rapat berhasil diubah!", meeting }),
      {
        status: 201,
        headers: { "Content-Type": "application/json" },
      }
