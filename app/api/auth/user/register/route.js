...
export async function POST(req) {
  try {
    ...
    const { email, name, nipp, unitId } = body;
    ...
    if (existingEmailUser || existingNippUser) {
      return NextResponse.json(
        { message: "Employee dengan Email dan NIPP tersebut sudah terdaftar." },
        { status: 400 }
      );
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      resetLink = `${process.env.NEXT_PUBLIC_API_KAI_OFFICE_BASE_URL}/setup-account?token=${resetToken}`;
      return user;
    });
    ...
    try {
      await sendEmail({
        to: email,
        subject:
          "🎉 Registrasi Akun Berhasil - Atur Password dan No Telp Anda!",
        html: `
      <!DOCTYPE html>
      <html lang="id">
      <head>
        ...
        <title>Registrasi Berhasil dan Perlu Setup lebih lanjut!</title>
      </head>
        ...
        <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
          ...
          <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 40px 30px; text-align: center;">
            ...
            <p style="color: #e8f0fe; margin: 10px 0 0 0; font-size: 16px;">
              Registrasi akun Anda berhasil tetapi memerlukan setup akun lebih lanjut!
            </p>
          </div>
    ...
    return NextResponse.json({
      ...
      message: "Registrasi akun berhasil, cek email anda untuk setup akun!",
      data: {
        userId: result.id,
        email: result.email,
        name: result.name,
